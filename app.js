...
function nextQuestion(){
  ...
  if(pool.length === 0){ alert("No verbs for this tense!"); return; }

  const verb = getRandom(pool);
  const pronoun = getRandom(Object.keys(verb.conjugations));
  currentCorrect = verb.conjugations[pronoun];

  document.getElementById('question').textContent = `Conjugate "${verb.infinitive}" (${verb.translation}) for "${pronoun}"`;
  const optionsDiv = document.getElementById('options');
  optionsDiv.innerHTML = '';

  // Get all unique conjugations excluding the correct one
  const uniqueConjugations = [...new Set(Object.values(verb.conjugations))].filter(a => a !== currentCorrect);

  // Pick up to 3 distractors (if fewer than 3 unique wrong answers exist, just use them all)
  const distractors = [];
